...
class GeneralController extends Controller
{
    ...
    public function general_busqueda_producto_b2b_data()
    {
    ...
    public function general_busqueda_producto_b2b_productos(Request $request)
    {
        $criterio = $request->input("criterio");
        $proveedor = $request->input("proveedor");

        $productos = DB::table("modelo_proveedor_producto")
            ->where("codigo_barra", $criterio)
            ->where("id_modelo_proveedor", $proveedor) # 4 es el ID del proveedor exel del norte
            ->get()
            ->toArray();
        ...
        foreach ($productos as $producto) {
            $producto->precioLista = round($producto->precioLista, 2);
            $producto->existencias = DB::table("modelo_proveedor_producto_existencia")
                ->select("modelo_proveedor_producto_existencia.*", "modelo_proveedor_almacen.id_locacion", "modelo_proveedor_almacen.locacion", "modelo_proveedor_almacen.estado")
                ->join("modelo_proveedor_almacen", "modelo_proveedor_producto_existencia.id_almacen", "=", "modelo_proveedor_almacen.id")
                ->where("modelo_proveedor_producto_existencia.id_modelo", $producto->id)
                ->where("modelo_proveedor_almacen.id_modelo_proveedor", $proveedor)
                ->get()
                ->toArray();

            if (count($producto->existencias) > 0) {
                $producto->ultima_actualizacion = $producto->existencias[0]->updated_at;
            }
        }
    ...
    public function general_busqueda_producto_b2b_actualizar(Request $request)
    {
        ...
        DB::table("modelo_proveedor_producto")
            ...
            ->where("id_modelo_proveedor", 4) # 4 es el ID del proveedor exel del norte
            ->update([
                "actualizar" => !$informacion->actualizar
            ]);
    ...
    public function general_reporte_venta_data(Request $request)
    {
        ...
        $vendedores = DB::select("SELECT
                                ...
                                WHERE nivel.id = 8 AND usuario.status = 1");
    ...
    public function general_reporte_venta_historial(Request $request)
    {
        ...
        foreach ($ventas as $venta) {
            ...
            if ($venta->area == "AGRO") {
                $usuario_agro =
                    DB::table('usuarios_agro')
                        ...
                        ->first();
                if ($usuario_agro) {
                    $venta->usuario_agro = $usuario_agro->nombre;
                }
            }
            $total = 0;
            $guias_array = array();
            ...
            //!AQUI VA LO DE CAMBIAR STATUS
            if ($venta->bd != 0 && $venta->fase == "Documento Terminado") {
            ...
            foreach ($productos as $index => $producto) {
                ...
                $sheet->setCellValue('X' . $contador_fila, $estatus_factura);
                ...
                if (!$venta->status) {
                    $spreadsheet->getActiveSheet()->getStyle('A' . $contador_fila . ':AI' . $contador_fila)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('DA5A5A');
                }
        ...
        $spreadsheet->getActiveSheet()->setAutoFilter("A" . $primera . ":AI" . $ultima);
    ...
    public function general_orden_compra_recepciones(Request $request)
    {
        ...
        $spreadsheet->getActiveSheet()->getStyle('A1:O1')->getFont()->setBold(1)->getColor()->setARGB('DE573A'); # Cabecera en negritas con color negro
    ...
    public function general_reporte_producto_costo_precio_promedio(Request $request)
    {
        ...
        if ($data->type == "venta") {
            $productos = DB::table("documento")
                ...
                ->where("documento.id_tipo", 2)
                ->where("documento.status", 1)
                ->where("empresa_almacen.id_empresa", $data->company)
                ->groupBy("modelo.sku")
                ->get()
                ->toArray();
        } else {
            $empresa_data = DB::table("empresa")->find($data->company);
        ...
        foreach ($productos as $producto) {
            ...
            if ($data->type == "venta") {
                $producto->precio_promedio_enero = self::rawinfo_precio_cantidad_mes("01", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_febrero = self::rawinfo_precio_cantidad_mes("02", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_marzo = self::rawinfo_precio_cantidad_mes("03", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_abril = self::rawinfo_precio_cantidad_mes("04", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_mayo = self::rawinfo_precio_cantidad_mes("05", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_junio = self::rawinfo_precio_cantidad_mes("06", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_julio = self::rawinfo_precio_cantidad_mes("07", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_agosto = self::rawinfo_precio_cantidad_mes("08", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_septiembre = self::rawinfo_precio_cantidad_mes("09", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_octubre = self::rawinfo_precio_cantidad_mes("10", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_noviembre = self::rawinfo_precio_cantidad_mes("11", $producto->id, $data->company, $data->year);
                $producto->precio_promedio_diciembre = self::rawinfo_precio_cantidad_mes("12", $producto->id, $data->company, $data->year);
    ...
    public function general_reporte_nota_credito_reporte(Request $request)
    {
        ...
        $company = DB::table("empresa")->find($data->company);
    ...
    */
    ...
    public function comprasReporteHp(Request $request)
    {
        set_time_limit(0);

        $data = json_decode($request->input('data'));
        $hpSkusArray = $this->getHpSkusArray();

        $filteredCompras = $this->getFilteredCompras($data, $hpSkusArray);

        $productos = $this->getProductos($filteredCompras);
        if (empty($productos)) {
            return response()->json([
                'code'  => 550,
                'message'   => 'Reporte Vacio'
            ]);
        }
        ...
        $excel = $this->processProductos($productos, $hpSkusArray, $date);

        return response()->json([
            'code' => 200,
            'excel' => $excel,
            'docs' => $filteredCompras
        ]);
    }
    ...
    public function ventasReporteHp(Request $request)
    {
        set_time_limit(0);

        $filteredVentas = array();

        $data = json_decode($request->input('data'));

        $hpSkusArray = $this->getHpSkusArray();

        $ventasHP = DB::table('movimiento')
            ->select('documento.*')
            ->join('modelo', 'movimiento.id_modelo', 'modelo.id')
            ->join('documento', 'movimiento.id_documento', 'documento.id')
            ->where('modelo.descripcion', 'like', '%hp%')
            ->where('modelo.cat2', 'hp')
            ->whereBetween('documento.created_at', [$data->fecha_inicio . ' 00:00:00', $data->fecha_final . ' 23:59:59'])
            ->where('documento.id_tipo', 2)
            ->where('documento.refacturado', 0)
            ->where('documento.id_fase', '>', 5)
            ->groupBy('documento.id')
            ->pluck('id');

        foreach ($ventasHP as $key) {
            ...
            array_push($filteredVentas, ...$ventas);
        }

        if (empty($filteredVentas)) {
            return response()->json([
                'code'  => 550,
                'message'   => 'Reporte Vacio'
            ]);
        }
        $excel = $this->processProductosVentas($filteredVentas, $hpSkusArray);
