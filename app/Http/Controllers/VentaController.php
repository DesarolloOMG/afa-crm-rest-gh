...
class VentaController extends Controller
{
    ...
    public function venta_venta_importacion_data(Request $request)
    {
        ...
        $marketplaces = DB::select("SELECT
                                    ...
                                    INNER JOIN area ON marketplace_area.id_area = area.id
                                    WHERE usuario_marketplace_area.id_usuario = " . $auth->id . "
                                    AND marketplace.marketplace IN('AMAZON', 'LINIO', 'CLAROSHOP','SEARS', 'WALMART','SANBORNS')");
        // AND marketplace.marketplace IN('AMAZON', 'LINIO', 'CLAROSHOP','SEARS', 'WALMART', 'TESTCLAROSHOP','TESTSEARS','TESTSANBORNS')");
    ...
    /* Venta > Promociones */
    public function venta_promocion_empresa(Request $request)
    {
        $auth = json_decode($request->auth);
        ...
        $empresas = DB::select("SELECT
                                ...
                                ORDER BY empresa.created_at ASC");
    ...
    public function venta_promocion_data($fecha_inicial, $fecha_final)
    {
        $promociones = DB::select("SELECT
                                        promocion.id,
                                        promocion.id_empresa,
                                        promocion.titulo,
                                        promocion.inicio,
                                        promocion.fin,
                                        empresa.empresa
                                    FROM promocion
                                    INNER JOIN empresa ON promocion.id_empresa = empresa.id
                                    WHERE promocion.inicio >= '" . $fecha_inicial . "' AND promocion.fin <= '" . $fecha_final . "' AND promocion.status = 1");
    ...
    public function venta_promocion_guardar(Request $request)
    {
        $data = json_decode($request->input("data"));
        $auth = json_decode($request->auth);

        $existe_promocion = DB::select("SELECT titulo FROM promocion WHERE titulo = '" . trim($data->titulo) . "' AND inicio = '" . $data->inicio . "' AND fin = '" . $data->fin . "' AND id != " . $data->id . "");

        if (!empty($promocion)) {
            return response()->json([
                "code" => 500,
                "message" => "Ya existe una promoción con el titulo '" . $data->titulo . "' y el rango de fechas proporcionado."
            ]);
        }

        if ($data->id) {
            $informacion_anterior = DB::select("SELECT * FROM promocion WHERE id = " . $data->id . "");

            if (empty($informacion_anterior)) {
                return response()->json([
                    "code" => 200,
                    "message" => "No se encontró la promoción, favor de contactar a un administrador."
                ]);
            }

            $informacion_anterior = $informacion_anterior[0];

            $productos_anteriores = DB::select("SELECT * FROM promocion_modelo WHERE id_promocion = " . $data->id . "");
            $informacion_anterior->productos = $productos_anteriores;

            DB::table("promocion_modelo")->where(["id_promocion" => $data->id])->delete();

            DB::table("promocion")->where(["id" => $data->id])->update([
            ...
            foreach ($data->productos as $producto) {
                ...
                DB::table("promocion_modelo")->insertGetId([
            ...
            $informacion_despues = DB::select("SELECT * FROM promocion WHERE id = " . $data->id . "");
            $informacion_despues = $informacion_despues[0];

            $productos_despues = DB::select("SELECT * FROM promocion_modelo WHERE id_promocion = " . $data->id . "");
            $informacion_despues->productos = $productos_despues;

            DB::table("promocion_edits")->insert([
                "id_usuario" => $auth->id,
                "id_promocion" => $data->id,
                "informacion_antes" => json_encode($informacion_anterior),
                "informacion_despues" => json_encode($informacion_despues)
            ]);
        } else {
            $promocion = DB::table("promocion")->insert([
                "id_usuario" => $auth->id,
        ...
        return response()->json([
            "code" => 200,
            "message" => $data->id ? "Promoción editada correctamente." : "Promoción creada correctamente."
        ]);
    }

    public function venta_promocion_eliminar($promocion_id, Request $request)
    {
        $auth = json_decode($request->auth);

        DB::table("promocion")->where(["id" => $promocion_id])->update([
            "status" => 0,
            "deleted_by" => $auth->id
        ]);
    ...
    /* Lino */
    public function venta_linio_publicaciones_data(Request $request)
    {
        $auth = json_decode($request->auth);

        $areas = self::areas_marketplaces($auth->id, "LINIO");
    ...
    public function venta_linio_publicaciones_guardar(Request $request)
    {
        $data = json_decode($request->input("data"));
        $auth = json_decode($request->auth);
    ...
    public function venta_linio_importar_ventas(Request $request)
    {
        $data = json_decode($request->input("data"));
        $auth = json_decode($request->auth);

        $response = LinioService::importarVentas($data->marketplace, $data->type, $auth->id, $data->initial_date, $data->final_date);
