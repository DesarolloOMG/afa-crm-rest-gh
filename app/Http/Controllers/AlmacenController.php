...
class AlmacenController extends Controller
{
    ...
    /* Almacen > prestamo */
    public function almacen_prestamo_generar_data()
    {
        ...
        $administradores = DB::select("SELECT
                                        ...
                                        FROM usuario
                                        INNER JOIN usuario_subnivel_nivel ON usuario.id = usuario_subnivel_nivel.id_usuario
                                        WHERE usuario_subnivel_nivel.id_subnivel_nivel IN (1, 3) # adminitrador del sistema y administrador de almacén
                                        AND usuario.id != 1");
        ...
        return response()->json([
            'code'  => 200,
            'empresas'  => $empresas,
            'adminitradores'    => $administradores
        ]);
    }

    public function almacen_prestamo_generar(Request $request)
    {
        $data = json_decode($request->input('data'));
        $auth = json_decode($request->auth);
        $series_afectadas = array();
        ...
        $bd = DB::select("SELECT
                        ...
                        FROM empresa_almacen
                        INNER JOIN empresa ON empresa_almacen.id_empresa = empresa.id
                        WHERE empresa_almacen.id = " . $data->almacen_entrada . "");
        ...
        $authy_user_id = DB::select("SELECT id FROM usuario WHERE authy = '" . $data->autorizado_por . "'");

        if (empty($authy_user_id)) {
            return response()->json([
                'code'  => 500,
                'message'   => "Usuario no encontrado, favor de contactar a un administrador." . " " . self::logVariableLocation()
            ]);
        }
        ...
        try {
            $verification = $authy_request->verifyToken($data->autorizado_por, $data->token);

            if (!$verification->ok()) {
                return response()->json([
                    'code'  => 400,
                    'message'   => "El token proporcionado es incorrecto." . " " . self::logVariableLocation()
                ]);
            }

            $id_almacen_entrada = DB::select("SELECT id_almacen, id_erp FROM empresa_almacen WHERE id = " . $data->almacen_entrada . "")[0];
            $id_almacen_salida  = DB::select("SELECT id_almacen, id_erp FROM empresa_almacen WHERE id = " . $data->almacen_salida . "")[0];

            # Verificar que las series existan
            foreach ($data->productos as $producto) {
                $existe_modelo = DB::select("SELECT id FROM modelo WHERE sku = '" . $producto->sku . "'");
                ...
                if ($producto->serie) {
                    foreach ($producto->series as $serie) {
                        ...
                        $existe_serie = DB::select("SELECT
                                                    ...
                                                    AND movimiento.id_modelo = " . $modelo . "");

                        if (empty($existe_serie)) {
                            return response()->json([
                                'code'  => 500,
                                'message'   => "La serie " . $serie . " no se encuentra en el sistema para su salida." . " " . self::logVariableLocation()
                            ]);
                        }

                        if ($existe_serie[0]->status == 0) {
                            return response()->json([
                                'code'  => 500,
                                'message'   => "La serie " . $serie . " no se encuentra disponible para su salida." . " " . self::logVariableLocation()
                            ]);
                        }

                        if ($existe_serie[0]->id_almacen != $id_almacen_salida->id_almacen) {
                            return response()->json([
                                'code'  => 500,
                                'message'   => "La serie " . $serie . " no se encuentra en el almacén especificado para su salida." . " " . self::logVariableLocation()
                            ]);
                        }
                    }
                }
            ...
            foreach ($data->productos as $producto) {
                ...
                if ($producto->serie) {
                    foreach ($producto->series as $serie) {
                        ...
                        $serie_afectada = new \stdClass();
                        $serie_afectada->id = $producto;
                        $serie_afectada->almacen_previo = $existe_serie[0]->id_almacen;
            ...
            $response_afectar = DocumentoService::afectarMovimiento($documento);

            return response()->json([
                'code'  => 200,
                'message'   => "Traspaso creado correctamente." . ($response_afectar->error) ? "No fue posible afectar el traspaso, favor de afectar manual, error: " . $response_afectar->mensaje . "" . " " . self::logVariableLocation() : "Traspaso afectado correctamente.",
                'documento' => $documento
            ]);
        } catch (\Authy\AuthyFormatException $e) {
            return response()->json([
                'code'  => 500,
                'message'   => "Token invalido, favor de verificar e intentar de nuevo. error " . $e->getMessage() . " " . self::logVariableLocation()
            ]);
        }
    }

    public function almacen_prestamo_regresar_data()
    {
        $solicitudes = $this->prestamos_raw_data(501);
    ...
    public function almacen_prestamo_regresar_guardar(Request $request)
    {
        $data = json_decode($request->input('data'));
        $auth = json_decode($request->auth);
        $series_afectadas = array();

        $id_almacen_entrada = DB::select("SELECT 
                                            empresa_almacen.id_almacen, 
                                            empresa_almacen.id_erp,
                                            documento.id_almacen_principal_empresa
                                        FROM documento
                                        INNER JOIN empresa_almacen ON documento.id_almacen_principal_empresa = empresa_almacen.id
                                        WHERE documento.id = " . $data->id . "")[0];

        $id_almacen_salida  = DB::select("SELECT 
                                            empresa_almacen.id_almacen, 
                                            empresa_almacen.id_erp,
                                            documento.id_almacen_secundario_empresa
                                        FROM documento
        ...
        foreach ($data->productos as $producto) {
            ...
            if ($producto->serie) {
                foreach ($producto->series as $serie) {
                    ...
                    if ($existe_serie[0]->id_almacen != $id_almacen_entrada->id_almacen) {
                        return response()->json([
                            'code'  => 500,
                            'message'   => "La serie " . $serie . " no se encuentra en el almacén especificado para su salida." . " " . self::logVariableLocation()
                        ]);
        ...
        $documento = DB::table('documento')->insertGetId([
            'id_almacen_principal_empresa'  => $id_almacen_salida->id_almacen_secundario_empresa,
            'id_almacen_secundario_empresa' => $id_almacen_entrada->id_almacen_principal_empresa,
            'id_tipo'                       => 10,
            'id_periodo'                    => 1,
            'id_cfdi'                       => 1,
            'id_marketplace_area'           => 8,
            ...
            'referencia'                    => 'N/A',
            'info_extra'                    => 'N/A',
            'autorizado_by'                 => $data->autorizado_by,
            'observacion'                   => $data->observacion . " - Traspaso de reversa generado por un prestamo en el CRM con el ID " . $data->id . ", primer traspaso con el ID " . $data->factura_folio . ".",
        ]);
        ...
        DB::table('documento')->where(['id' => $data->id])->update(['id_fase' => 502]);
        ...
        return response()->json([
            'code'  => 200,
            'message'   => $response_afectar->error ? "No fue posible afectar el traspaso, favor de afectar manual, error: " . $response_afectar->mensaje . " " . self::logVariableLocation() : "Mercancia regresada correctamente.",
            'documento' => $documento
        ]);
    }

    public function almacen_prestamo_historial_data($fecha_inicial, $fecha_final)
    {
        $solicitudes = $this->prestamos_raw_data(0, "AND documento.created_at BETWEEN '" . $fecha_inicial . " 00:00:00' AND '" . $fecha_final . " 23:59:59'");
    ...
    public function almacen_prestamo_historial_guardar(Request $request)
    {
        $data = json_decode($request->input('data'));
        $auth = json_decode($request->auth);
