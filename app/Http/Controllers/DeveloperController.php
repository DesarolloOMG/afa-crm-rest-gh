...
class DeveloperController extends Controller
{
    ...
    public function liberarExistenciasProducto(Request $request)
    {
        $producto = $request->input("sku");

        $documentos = DB::select('CALL obtener_movimientos_por_sku(?)', [$producto]);
        ...
        foreach ($documentos as $documento) {
            $bd = DB::table('documento')
                ...
                ->where('documento.id', $documento->id_documento)
                ->first();

            $tiene_factura = ComodinService::validar_factura($bd->bd, $documento->id_documento);

            if ($tiene_factura) {
                DB::table('documento')->where('id', $documento->id_documento)->update(['id_fase' => 6]);
            }
        }

        return response()->json([
            'code' => 200,
            'message' => "Existencias liberadas con exito."
        ]);
    }
    ...
    public function temporal1(): JsonResponse
    {
        set_time_limit(0);
        ...
        $documentos = DB::table('documento')
            ...
            ->join('marketplace', 'marketplace_area.id_marketplace', '=', 'marketplace.id')
            ->where('marketplace_area.id_area', 4)
            ->where('documento.created_at', '>', '2025-03-18 00:00:00')
            ->where('documento.id_tipo', 2)
            ->where('documento.status', 1)
            ->orderBy('documento.id_marketplace_area')
            ->get()
            ->toArray();
        ...
        if (!empty($documentos)) {
            foreach ($documentos as $documento) {
                ...
                $documento_data = @json_decode(file_get_contents(config('webservice.url') .'6/Factura/Estado/Folio/' . $documento->id));
                ...
                if (!empty($documento_data)) {
                    DB::table('documento')->where('id', $documento->id)->update([
                        'documento_extra'=> $documento_data->documentoid,
                        'factura_serie' => $documento_data->serie,
                        'factura_folio' => $documento_data->folio,
                        'uuid' => $documento_data->uuid,
                    ]);
    ...
    //!ZONA DE Temporal END (MANTENER AL FINAL)
}
